<template>
	<div v-for="data in datas.docs" :key="data.id" class="dark:bg-gray-800 dark:text-gray-50">
		<div class="container grid grid-cols-12 mx-auto dark:bg-gray-900">
      <div 
        class="bg-no-repeat bg-cover dark:bg-gray-700 col-span-full lg:col-span-4" 
        :style="{
          ...
          backgroundSize: 'cover'
			...
			<div class="flex flex-col p-6 col-span-full row-span-full lg:col-span-8 lg:p-10">
				<div class="flex justify-start">
					<span class="px-2 py-1 text-xs rounded-full dark:bg-violet-400 dark:text-gray-900">{{ data.label }}</span>
				</div>
				<h1 class="text-3xl font-semibold">{{ data.name }}</h1>
				<p class="flex-1 pt-2">{{ data.description }}</p>
				...
				<div class="flex items-center justify-between pt-2">
					<div class="flex space-x-2">
						...
						<span class="self-center text-sm">by {{ data.author.value.nama }}</span>
					</div>
...
<script setup>
const datas = await $fetch('http://localhost:3000/api/Blog')
</script>